/**
 * D1 마이그레이션 검증 테스트
 * 성적 입력, 저장, 미리보기 탭 이동 후 데이터 유지 확인
 */
...
async function test() {
  ...
  try {
    console.log('\n');
    console.log('════════════════════════════════════════════════════════');
    console.log('🧪 D1 마이그레이션 검증 E2E 테스트');
    console.log('════════════════════════════════════════════════════════');
    console.log('');
    ...
    const stillHasData = pageText2.includes('90') || pageText2.includes('85');
    console.log(`${stillHasData ? '✅' : '❌'} 데이터 유지: ${stillHasData ? '데이터 유지됨' : '데이터 손실됨'}`);
    console.log('');

    // 최종 결과
    console.log('════════════════════════════════════════════════════════');
    if (hasScore && stillHasData) {
      console.log('✅ D1 마이그레이션 성공!');
    } else {
      console.log('❌ D1 마이그레이션 테스트 실패');
    }
    console.log('════════════════════════════════════════════════════════');
    console.log('');
