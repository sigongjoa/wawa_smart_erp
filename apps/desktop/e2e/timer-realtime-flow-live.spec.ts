...
test.describe('Timer Realtime Flow — v1.9.0 복원', () => {
  test('대기 카드 클릭 → check-in → pause → resume → check-out', async ({ page }) => {
    ...
    await page.waitForSelector('.v2-timer-shell', { timeout: 10000 });

    // 로딩 스피너 사라질 때까지
    await page.waitForSelector('.rpt-loading', { state: 'detached', timeout: 10000 }).catch(() => {});

    // 요약 바에 있는 대기 셀 숫자 확인
    const waitingCell = page.locator('.v2-summary-cell:has(.v2-summary-cell__label:text-is("대기")) .v2-summary-cell__value');
    const waitingCount = parseInt((await waitingCell.textContent()) || '0', 10);
    console.log('대기 학생 수:', waitingCount);
    expect(waitingCount).toBeGreaterThan(0); // 담당 학생 21명 중 오늘 요일에 맞는 학생

    // 첫 대기 카드 선택
    const firstWaitingCard = page.locator('[data-testid^="waiting-card-"]').first();
    await expect(firstWaitingCard).toBeVisible();
    const studentName = await firstWaitingCard.locator('.v2-pending-row__name').first().textContent();
    const studentNameClean = studentName?.trim().split(/\s+/)[0] || '';
    console.log('체크인 대상 학생:', studentNameClean);

    // confirm 자동 승인 (체크아웃 단계에서 사용됨)
    page.on('dialog', (dialog) => dialog.accept());

    // ─── 1) 체크인 ───────────────────
    await firstWaitingCard.locator('.v2-pending-row__main').first().click();
    await page.waitForTimeout(1500);

    // 세션 카드가 우측에 나타났는지
    const sessionCard = page.locator('.v2-timer-card').first();
    await expect(sessionCard).toBeVisible({ timeout: 10000 });
    await expect(sessionCard.locator('.v2-timer-card__name')).toContainText(studentNameClean);

    // 카운트다운 값 표시 확인
    const timerValue = sessionCard.locator('.v2-timer-card__time').first();
    ...
    await expect(pausedCard.locator('.v2-pill--warning:has-text("정지")')).toBeVisible();
    ...
    // 다시 활성 카드로
    const activeAgain = page.locator('.v2-timer-card:not(.v2-timer-card--paused)').first();
    ...
    const completedCell = page.locator('.v2-summary-cell:has(.v2-summary-cell__label:text-is("완료")) .v2-summary-cell__value');
    const completedCount = parseInt((await completedCell.textContent()) || '0', 10);
