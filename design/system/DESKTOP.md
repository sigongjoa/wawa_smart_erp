...
#### `<Modal>` (`apps/desktop/src/components/Modal.tsx`)
완전 구조형 — Header / Body / Footer 컨벤션 고정.
...
자동 처리: `role="dialog"` + `aria-modal` + Tab 포커스 트랩 + Escape 키 + 닫기 버튼.

#### `<DialogShell>` (`apps/desktop/src/components/DialogShell.tsx`)
얇은 래퍼 — 자체 마크업이 있는 모달용. ARIA + Escape만 추가.

```tsx
<DialogShell
  ariaLabel="PIN 재설정"
  onClose={() => setOpen(false)}
  overlayClassName="gacha-modal-overlay"  // 기본은 'modal-overlay'
>
...
자동 처리: `role="dialog"` + `aria-modal="true"` + `aria-label` + Escape 키 + 오버레이 클릭 닫기.

#### 사용 가이드

- **신규 모달**: `<Modal>` 우선 (헤더/바디/푸터 구조 일관)
- **기존 마크업 보존 필요**: `<DialogShell>` 래핑
- **금지**: `<div className="modal-overlay">` 직접 사용 — ARIA 누락·Escape 누락 위험

#### `useConfirm()` (`apps/desktop/src/components/Toast.tsx`)
native `confirm()` 대체 — `await`로 사용:
...
**금지**: `window.confirm()` / `alert()` — 모두 OS·브라우저별 다름, 게임 폴리시 깨짐.
