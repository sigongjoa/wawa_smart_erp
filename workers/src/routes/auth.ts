...
// 토큰 갱신
authRouter.post('/refresh', async (request: Request, env: any) => { const context = env as RequestContext;
  const ipAddress = context.request.headers.get('CF-Connecting-IP') || 'unknown';
...
// 로그아웃
authRouter.post('/logout', async (request: Request, env: any) => { const context = env as RequestContext;
  try {
    if (!context.auth) {
      return unauthorizedResponse();
    }
