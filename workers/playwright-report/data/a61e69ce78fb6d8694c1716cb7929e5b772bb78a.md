...
- Name: timer.spec.ts >> Timer API E2E Tests >> should return 400 when recording attendance with invalid data
- Location: e2e/timer.spec.ts:80:3
...
> 91  |     expect(response.status()).toBe(400);
  ...
  122 |   });
  123 | 
  124 |   test('should validate attendance recording requires instructor or admin role', async ({ request }) => {
  125 |     const response = await request.post('/api/timer/attendance', {
  126 |       headers: {
  127 |         'Authorization': 'Bearer student-token',
  128 |       },
  129 |       data: {
  130 |         studentId: 'student-123',
  131 |         classId: 'class-123',
  132 |         date: '2026-04-08',
  133 |         status: 'present',
  134 |       },
  135 |     });
  ...
  139 |     expect(data.success).toBe(false);
  140 |   });
  141 | });
  142 | 
```